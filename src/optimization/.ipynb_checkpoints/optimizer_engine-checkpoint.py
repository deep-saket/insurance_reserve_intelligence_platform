...
class OptimizationEngine:
    ...
    def _features_from_policy(self, policy: Policy) -> torch.Tensor:
        ...
        return torch.tensor(
            [[
                0.0,
                float(policy.age),
                policy.pricing_interest_rate,
                policy.scenario_interest_rate,
                policy.premium / max(float(policy.sum_assured), 1.0),
                policy.sum_assured,
    ...
    def target_reserve_optimization(self, policy: Policy, target_reserve: float) -> OptimizationResult:
        ...
        def objective(interest_rate: float) -> float:
            ...
            features[:, FEATURE_INDEX["scenario_interest_rate"]] = interest_rate
    ...
    def premium_optimization(self, policy: Policy) -> OptimizationResult:
        ...
        for _ in range(self.config.steps):
            ...
            features = self._features_from_policy(policy).clone()
            features[:, FEATURE_INDEX["premium_ratio"]] = premium / max(float(policy.sum_assured), 1.0)
        ...
        optimized_features[:, FEATURE_INDEX["premium_ratio"]] = premium.detach() / max(float(policy.sum_assured), 1.0)
    ...
    def constrained_premium_optimization(self, policy: Policy) -> OptimizationResult:
        ...
        def objective(values: np.ndarray) -> float:
            ...
            features = self._features_from_policy(policy).clone()
            features[:, FEATURE_INDEX["premium_ratio"]] = premium_value / max(float(policy.sum_assured), 1.0)
            reserve = float(self._predict(features).detach().cpu().item())
